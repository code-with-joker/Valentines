...
function startGame() {
    ...
    // ▶️ start background music when game starts
...
yesBtn.addEventListener('click', () => {
    ...
    const bgMusic = document.getElementById("bg-music1");
    if (bgMusic) {
        bgMusic.pause();
    ...
    const celebrationMusic = document.getElementById("bg-music");
    if (celebrationMusic) {
...
function moveNoButton() {
    ...
    const note = funnyNotes[Math.floor(Math.random() * funnyNotes.length)];

    noBtn.style.position = 'fixed';
    noBtn.style.left = `${x}px`;
    noBtn.style.top = `${y}px`;
    noBtn.title = note;
    noBtn.innerText = note;
}
...
function triggerConfetti() {
    ...
    // emoji falling effect (like hearts)
